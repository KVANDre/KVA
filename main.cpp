...
void getElement(const Image& obj, int x, int y) {
	if (((x < obj.getM()) || (x >= 1)) && ((x < obj.getN()) || (x >= 1))) std::cout << obj(x, y) << std::endl;
...
int main() {
	...
	paint.setM(utils::read_int("Input M(1, 1000): ", 1, 1000));
	paint.setN(utils::read_int("Input N(1, 1000): ", 1, 1000));
	fillOutRand(paint);
	...
	while (k != 0){
		...
		case 3: { 
			...
			Image paint2;
			paint2.setM(paint.getM());
			paint2.setN(paint.getN());
			fillOutRand(paint2);
